...
function activate(ctx) {
    ...
    const createBackgroundPanel = () => {
        ...
        backgroundPanel = vscode.window.createWebviewPanel('bulbasaurBackground', 'Bulbasaur Background', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    statusBarItem.show();
    // Register commands
    ctx.subscriptions.push(vscode.commands.registerCommand('eunitheme.showBackground', () => {
        createBackgroundPanel();
    }));
    ctx.subscriptions.push(vscode.commands.registerCommand('eunitheme.generateThemes', () => {
        const { exec } = require('child_process');
        exec('node theme-adapter.js all', { cwd: ctx.extensionPath }, (error) => {
            if (error) {
    ...
    ctx.subscriptions.push(vscode.commands.registerCommand('eunitheme.generateVisuals', () => {
        const { exec } = require('child_process');
        exec('node visual-enhancer.js', { cwd: ctx.extensionPath }, (error) => {
            if (error) {
